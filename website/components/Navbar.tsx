...
import mascot from "/public/assets/me.png"

const Navbar = () => {
  const [nav, setNav] = useState(false);
  const [shadow, setShadow] = useState(false);
  const [navBg, setNavBg] = useState('#1e1e1e');
  const [linkColor, setLinkColor] = useState('#fffdde');
  ...
  return (
    ...
    <div style={{ backgroundColor: `${navBg}` }} className={'fixed w-full h-20 z-[100]' + (shadow ? ' shadow-md shadow-gray-600' : '')}>
      ...
      <div className={nav ? 'fixed md:hidden left-0 top-0 w-full h-screen bg-black/70' : ''} onClick={(event) => { if (event.target === event.currentTarget) setNav(false) }}>
        ...
        <div className={'flex flex-col ' + (nav ? 'fixed left-0 top-0 w-[75%] sm:w-[60%] md:w-[45%] h-screen bg-[#1e1e1e]' : 'fixed left-[-100%] top-0') + ' p-10 ease-in duration-500'}>
          <div>
            <div className='flex w-full items-center justify-between'>
              <Image src={mascot} alt="/" width={45} height={50} />
              <div onClick={handleNav} className='cursor-pointer'>
                <AiOutlineClose size={30} />
              </div>
            </div>
            <div className='border-b border-[#fffdde] my-4 flex flex-col items-center justify-center'>
            ...
            <div className='py-4 flex flex-col'>
              ...
              <div className='inset-x-0 bottom-0'>
                <p className='pt-4 uppercase tracking-widest'> Social Media </p>
